...
#include "freertos/task.h" // 新增：任务通知相关头文件
...
static void disp_flush_wait(lv_disp_drv_t *disp_drv);
...
lv_disp_drv_t disp_drv; // 全局变量，供DMA中断回调使用
...
__attribute__((section(".iram1.0")))void spi_post_cb(spi_transaction_t *trans)
{
    // 回调逻辑：比如释放 LCD 总线忙标记
    lcd_bus_busy = 0;
...
{
	if (!lcd_bus_busy) {
        lv_disp_flush_ready(disp_drv); // 安全地通知 LVGL 刷新完成
    }
