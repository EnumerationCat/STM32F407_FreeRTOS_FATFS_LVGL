 ...
 * @param  len: 数据长度（字节数）
 * @retval 无
 * @note   异步DMA传输，不阻塞CPU，由中断完成信号量同步
 */
void LCD_WR_DATA_Array(uint8_t *data, uint16_t len)
{
    // 注意：CS在disp_flush/中断中控制，这里不操作，避免冲突
    HAL_SPI_Transmit_DMA(&hspi1, data, len);
    // 🔥 删除：while等待 + HAL_Delay，这是DMA性能杀手！
}
...
void LCD_Clear(uint16_t color)
{
    ...
    for(i = 0; i < count; i++)
    {
        HAL_SPI_Transmit_DMA(&hspi1, color_data, 2);
    }
...
void LCD_Fill(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend, uint16_t* color_buf)
{
    ...
    // 关键：整块数据一次性传输，不循环！
        ...
        LCD_CS_Clr();
        // 阻塞模式：整块发送
        HAL_SPI_Transmit(&hspi1, (uint8_t*)color_buf, data_len, HAL_MAX_DELAY);
        LCD_CS_Set();
    #endif

    #if SPI_DMA_Mode
		LCD_CS_Clr(); // 修复：DMA启动时拉低CS
        // DMA模式：调用批量DMA函数，CS由中断控制
        LCD_WR_DATA_Array((uint8_t*)color_buf, data_len);
    #endif
}
/**
